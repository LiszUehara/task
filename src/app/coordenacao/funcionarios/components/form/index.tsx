...
export function NewFuncionarioForm({userId}: {userId: string} ) {
  ...
  async function handleRegisterCustomer(data: FormData) {
    const response = await api.post("/api/funcionario", {
      name: data.name,
      phone: data.phone,
      email: data.email,
      cargo: data.cargo,
  ...
  return (
    ...
    <section className="flex gap-2 my-2 flex-col sm:flex-row">
      ...
      <div className="flex-1">
        <label className="mb-1 text-lg font-medium">Cargo</label>
        <Input
          type="text"
          name="cargo"
    ...
    <button
      type="submit"
      className="bg-blue-900 my-4 px-2 h-11 rounded text-white font-bold"
    >
      Cadastrar
    </button>
